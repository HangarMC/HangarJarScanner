...
public class StringLiteralSubstringCheck implements ClassCheck {
    ...
    public ClassCheckResult check(ClassNode classNode) {
        for (FieldNode fieldNode : classNode.fields) {
            ...
            if (banned != null) {
                return new ClassCheckResult(Severity.HIGHEST, classNode, "contains banned string literal substring '" + banned + "' in field " + fieldNode.name);
            }
        }
        for (MethodNode methodNode : classNode.methods) {
            var banned = findBannedSubstring(methodNode);
            if (banned != null) {
                return new ClassCheckResult(Severity.HIGHEST, classNode, "contains banned string literal substring '" + banned + "' in method " + methodNode.name);
            }
        }
        return null;
    }
