    ...
    public MethodCheckResult check(MethodInsnNode insnNode, MethodNode methodNode, ClassNode classNode) {
        for (String bannedWord : bannedWords) {
            if (insnNode.name.toLowerCase(Locale.ROOT).contains(bannedWord)) {
                return new MethodCheckResult(Severity.HIGHEST, name(), methodNode, classNode, "calls method with banned word " + bannedWord + ": " + insnNode.name);
            } else if (methodNode.name.toLowerCase(Locale.ROOT).contains(bannedWord)) {
                return new MethodCheckResult(Severity.HIGHEST, name(), methodNode, classNode, "method contains banned word " + bannedWord);
            }
        }
        return null;
    }
